U1RegisterAddon("TinyInspect", {
    ...
    .. "`有爱整合此插件取代了之前的背包装等、鼠标提示装等、装备边框染色等好几个插件。为了一些风格的统一，稍微改动了一下默认设置和配色等小地方，在此对原作者表示感谢。",
    nopic = 1,
    ...
    {
        text = "团队面板",
        tip = "说明`启用团队装等选项后，/ti raid可以显示团员的装等列表，不过有爱用户也可以使用团员信息统计",
        callback = function(cfg, v, loading) SlashCmdList.TinyInspect("raid") end,
    },
    {
        text = "恢复默认设置",
        tip = "说明`有爱整合的TinyInspect和原有插件功能保持一致，只是调整了一些默认值，如果你之前使用过单体的TinyInspect，建议重置一下选项。",
