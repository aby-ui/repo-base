...
--if AbyUpdateTooltipWrapperFunc then Item.UpdateTooltip = AbyUpdateTooltipWrapperFunc(Item.UpdateTooltip, 0.5) end --no longer needed, b.UpdateTooltip = Item.ShowTooltip, not cached.
...
function Item:UpdateSearch()
	...
	if self.hasItem then
		if not self.info.cached then
			linkOrLocation = self:GetQuery()
		else
			linkOrLocation = self:GetInfo().link
		end
	end
	local matches = search == '' or self.hasItem and Search:Matches(linkOrLocation, search)
