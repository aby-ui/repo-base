...
U1RegisterAddon("!!Forwarder", { dummy = 1,
    ...
    desc = "在野外也可以看到组队频道和交易频道的信息，插件由有爱warbaby原创奉献",
...
--     desc = "很多单体插件使用了地图数据（LibMapData）这个库，但由于更新不及时，玩家到新地图上时会不停的刷屏。`有爱特别制作了一个最新版的库，当遇到插件刷MapData Missing的时候启用即可。`如果没有单体插件或没有使用这个库的单体插件，可以完全关闭或删除此插件。",
--     alwaysRegister = 1,
--     defaultEnable = 1,
--     tags = { TAG_MANAGEMENT }
-- })
