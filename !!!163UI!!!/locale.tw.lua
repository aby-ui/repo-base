...
L["TAG_BIG"] = "大型插件"
L["TAG_DESC_BIG"] = "此分類下是有愛整合包中記憶體佔用相對較高、模組數量較多的大型插件，而且關閉後一般不影響正常的遊戲習慣，如果希望節省記憶體可以全部停用。"
L["TAG_TRADING"] = "商業交易"
...
L["TAG_DESC_MANAGEMENT"] = "此分類是用來'管理插件的插件'，主要是一些比較高階的錯誤資訊及資料統計等，有愛控制台就在此分類中，此外還有錯誤提示增強、錯誤消息屏蔽等功能。"
L["TAG_ITEM"] = "物品裝備"
...
--[[1658]] L["只有有爱官方支持的插件才能用拼音搜索名称。"] = "繁體版不支持拼音檢索"
...
--[[1754]] --L["有爱"] = "有愛"
--[[1771]] L["有爱插件中心"] = "有愛插件中心"
--[[1773]] L["    有爱（163UI）是新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"] = "    有愛（163UI）是新一代整合插件。其設計理念是兼顧整合插件的易用性和單體插件的靈活性，同時適合普通和高級用戶群體。|n|n    功能上，有愛實現了任意插件的隨需載入，並可先進入遊戲再逐一載入插件，此為全球首創。此外還有標籤分類、拼音檢索、介面縮排等特色功能。"
...
--[[   3]] --L["有爱"] = "有愛"
--[[   5]] L["有爱是新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"] = "有愛（163UI）插件是新一代整合插件。其設計理念是兼顧整合插件的易用性和單體插件的靈活性，同時適合普通和高級用戶群體。``插件中心控制台是有愛團隊全力打造的管理介面，整合了'插件載入管理'、'插件選項配置'、'標籤式分類'、'即時全文檢索搜尋'、'小地圖按鈕收集'等一系列先進功能。而且任意插件均可'一鍵啟用'，不需重載介面。``有愛將依靠強大的技術實力，讓插件更少的報錯、讓問題更快的回復、讓建議更快的實現，為您提供更新更快更強大的新一代整合插件。"
--[[   9]] L["|cffcd1a1c[有爱原创]|r"] = "|cffcd1a1c[有愛原創]|r"
--[[  13]] L["延迟加载插件"] = "延遲加載插件"
--[[  14]] L["说明`有爱独家支持，可以先读完蓝条然后再逐一加载插件。会大大加快读条速度，但是加载大型插件时会有卡顿。如果不喜欢这种方式，请取消勾选即可，下次进游戏时就会采用新设置。` `对比测试：`未开启时，在第7.5秒后读完蓝条同时加载完全部插件`开启后，在第3.8秒读完蓝条，第8.0秒加载完全部插件"] = "說明`有愛獨家支援，可以先讀完藍條然後再逐一載入插件。會大大加快讀條速度，但是載入大型插件時會有卡頓。如果不喜歡這種方式，請取消勾選即可，下次進遊戲時就會採用新設置。` `對比測試：`未開啟時，在第7.5秒後讀完藍條同時載入完全部插件`開啟後，在第3.8秒讀完藍條，第8.0秒載入完全部插件"
