...
L["TAG_BIG"] = "大型插件"
L["TAG_DESC_BIG"] = "此分类下是有爱整合包中内存占用相对较高、模块数量较多的大型插件，而且关闭后一般不影响正常的游戏习惯，如果希望节省内存可以全部停用。"
L["TAG_TRADING"] = "商业交易"
...
L["TAG_DESC_MANAGEMENT"] = "此分类是用来'管理插件的插件'，主要是一些比较高阶的错误信息及数据统计等，有爱控制台就在此分类中，此外还有错误提示增强、错误消息屏蔽等功能。"
L["TAG_ITEM"] = "物品装备"
...
--[[1658]] L["只有有爱官方支持的插件才能用拼音搜索名称。"] = true
...
--[[1754]] --L["有爱"] = true
--[[1771]] L["有爱插件中心"] = true
--[[1773]] L["    有爱（163UI）是新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"] = true
...
--[[   3]] --L["有爱"] = true
--[[   5]] L["有爱是新一代整合插件。其设计理念是兼顾整合插件的易用性和单体插件的灵活性，同时适合普通和高级用户群体。|n|n    功能上，有爱实现了任意插件的随需加载，并可先进入游戏再逐一加载插件，此为全球首创。此外还有标签分类、拼音检索、界面缩排等特色功能。"] = true
--[[   9]] L["|cffcd1a1c[有爱原创]|r"] = true
--[[  13]] L["延迟加载插件"] = true
--[[  14]] L["说明`有爱独家支持，可以先读完蓝条然后再逐一加载插件。会大大加快读条速度，但是加载大型插件时会有卡顿。如果不喜欢这种方式，请取消勾选即可，下次进游戏时就会采用新设置。` `对比测试：`未开启时，在第7.5秒后读完蓝条同时加载完全部插件`开启后，在第3.8秒读完蓝条，第8.0秒加载完全部插件"] = true
